...
- **Current Version**: 4.13.1 (Release Package: **5.0.0**, Report Format: 3.2)
- **Release Status**: Assessment Engine **feature-complete & stable** (engine logic frozen); professional HTML report redesign completed (v4.12.1), **professional GUI redesign completed** (v4.13.0, approved), **final UI polish & usability review completed** (v4.13.1, approved), and **Packaging & Distribution completed** (v5.0.0: source + portable Windows exe). Next: **Final Release Preparation** — Website, Gumroad, marketing assets, v5.0 release candidate. No architectural/engine changes unless a verified defect is found.
...
| ID | Issue | File | Fix |
|----|-------|------|-----|
| B14 | Verdict reported "Critical/High Risk" for a warnings-only scan (0 confirmed vulnerabilities). Warning findings inherit the module base severity (e.g. a SQLi *warning* is CRITICAL), and `_overall_verdict` / legacy `get_overall_severity` counted them by severity, so warnings alone escalated the verdict | `core/assessment_engine.py`, `core/finding.py` | Verdict severity buckets now filter to confirmed vulnerability findings only (`get_vulnerabilities()`); warnings resolve to the informational **'Warning only'** tier. Added validation §SOP #1 checks |
| B15 | Risk score overstated warnings: a warnings-only scan (4 warnings, 0 vulns) scored 19% / B+ (SOP Issue #5). The old formula normalized warnings against their own half severity weight (`max_possible += sev_weight*2`), which is mathematically invariant to `WARNING_SEVERITY_FACTOR` and let warnings move the score onto the severity scale | `core/risk_engine.py`, `core/decision_engine.py`, `core/assessment_config.py` | Warnings now contribute a **bounded ~0.5-point load each** (`WARNING_UNIT`, capped by `WARNING_LOAD_CAP`) added to the numerator but **excluded from `max_possible`**, so they never dominate and a warnings-only scan reads ~2% / A+. Vulnerability path unchanged (byte-identical for vuln-only scans). Mirrored in `RiskCalculator`. Gates: validation 0/0, engine 0/0, `REGRESSION=0` (all shifts `[EXPLAINED] risk_from_band_shift`/`overall_shift`), `PARITY=0` |
